...
from .knowledge_loader import search_knowledge, load_all_knowledge


class ConversationViewSet(viewsets.ModelViewSet):
    """
    SUPER FAST Production Version
    Optimized for Ollama + DRF + React
    """
    ...
    def chat(self, request, pk=None):
        ...
        # -------------------------------------------------
        # FAST KNOWLEDGE SEARCH (only relevant data)
        # -------------------------------------------------
        # knowledge = search_knowledge(user_text)
        knowledge = load_all_knowledge()

        # -------------------------------------------------
        # SHORT SYSTEM PROMPT (faster than huge prompt)
        # -------------------------------------------------
        ollama_messages = [
            {
                "role": "system",
...
Use this company knowledge if relevant:

{knowledge}

Rules:
- If found in knowledge, answer accurately.
- If not found, say politely unavailable.
...
"""
        ...
        # -------------------------------------------------
        # ONLY LAST 4 VALID MESSAGES (huge speed boost)
        # -------------------------------------------------
        recent_messages = (
        ...
        try:
            ...
            return Response(
                {"detail": f"Ollama failed: {str(exc)}"},
